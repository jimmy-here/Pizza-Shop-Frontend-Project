...
const PizzaCard = ({ pizza }) => {
  ...
  return (
    <div className={`card mb-3 ${isDelayed ? 'border-danger' : 'border-primary'}`}>
      <div className="card-body">
        <h5 className="card-title">Order ID: {formattedOrderId}</h5>
        {pizza.stage !== 'Order Picked' && (
          <p className="card-text">Total time: {formatTime(pizza.totalTime)}</p>
        )}
        {pizza.stage === 'Order in Making' && (
          <p className="card-text">Remaining time: {formatTime(pizza.remainingTime)}</p>
        )}
        <div className="d-flex justify-content-between mt-3">
          ...
          {pizza.stage !== 'Order Picked' && (
            <button className="btn btn-outline-primary" onClick={handleNextStage}>
              {pizza.stage === 'Order Ready' ? 'Next' : 'Next Stage'}
            </button>
          )}
          {pizza.stage === 'Order Picked' && (
            <button className="btn btn-success" onClick={handleOrderPicked}>
              Order Picked
            </button>
          )}
        </div>
